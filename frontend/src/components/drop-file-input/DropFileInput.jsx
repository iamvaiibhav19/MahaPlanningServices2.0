...
const DropFileInput = (props) => {
  ...
  const { form, setForm, displayFinish, setDisplayFinish, handleFinish, leadUploadId, setLeadUploadId } = props;
  ...
  const handleAnotherDoc = () => {
    ...
    axios
      .post(`https://mahaplanningservices.herokuapp.com/api/v1/lead/upload/${formId}`, formData, {
        headers: {
      ...
      {value ? (
        ...
        <>
          ...
          <Button
            ...
            onClick={!uploadAnother ? handleUploadFinish : handleAnotherDoc}
            disabled={fileList.length === 0}
            style={{
              margin: '20px auto',
            }}
          >
            {uploadAnother ? 'Upload More' : 'Upload & Submit'}
          </Button>
          {fileList.length === 0 && !uploadAnother && (
            <Button
              variant="contained"
              sx={{ color: 'white', backgroundColor: '#3f51b5' }}
